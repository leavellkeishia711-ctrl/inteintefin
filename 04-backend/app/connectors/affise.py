...
class AffiseConnector(Connector):
    def __init__(self, config: Any, decrypted_api_key: str):
        super().__init__(config)
        self.api_key = decrypted_api_key
        settings = getattr(config, 'settings', {}) or {}
        self.base_url = settings.get("base_url", "https://api.affise.com/3.0").rstrip("/")

    def _get_headers(self) -> Dict[str, str]:
        """Affise API uses API-Key header for authentication."""
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await with_retry(lambda: client.get(
                    f"{self.base_url}/offers?limit=1",
                    headers=headers,
                    timeout=10
                ))
                ...
                logger.error(f"Affise test_connection HTTP error: {type(e).__name__} status={e.response.status_code}")
                return False
            except Exception as e:
                logger.error(f"Affise test_connection network error: {type(e).__name__}")
                return False

    async def fetch_campaigns(self) -> List[Dict[str, Any]]:
        """Fetches raw campaign/offer list."""
        async with httpx.AsyncClient() as client:
            headers = self._get_headers()
            response = await with_retry(lambda: client.get(
    ...
    async def fetch_metrics(self) -> List[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient() as client:
            headers = self._get_headers()
            response = await with_retry(lambda: client.get(
                f"{self.base_url}/stats/custom?slice[]=offer",
                headers=headers,
                timeout=15
            ))
            response.raise_for_status()
