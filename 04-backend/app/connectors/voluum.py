...
class VoluumConnector(Connector):
    def __init__(self, config: Any, decrypted_api_key: str):
        super().__init__(config)
        self.api_key = decrypted_api_key
        settings = getattr(config, 'settings', {}) or {}
        self.base_url = settings.get("base_url", "https://panel-api.voluum.com").rstrip("/")

    def _get_headers(self) -> Dict[str, str]:
        if ":" in self.api_key:
            parts = self.api_key.split(":", 1)
            return {"Access-Key": parts[0], "Access-Token": parts[1]}
        return {"cwauth-token": self.api_key}
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await with_retry(lambda: client.get(
                    f"{self.base_url}/campaign?limit=1",
                    headers=headers,
                    timeout=10
                ))
                ...
                logger.error(f"Voluum test_connection HTTP error: {type(e).__name__} status={e.response.status_code}")
                return False
            except Exception as e:
                logger.error(f"Voluum test_connection network error: {type(e).__name__}")
                return False
    ...
    async def fetch_metrics(self) -> List[Dict[str, Any]]:
        async with httpx.AsyncClient() as client:
            headers = self._get_headers()
            response = await with_retry(lambda: client.get(
                f"{self.base_url}/report?groupBy=campaign&columns=campaignId,cost,revenue,clicks,visits,conversions",
                headers=headers,
                timeout=15
            ))
            response.raise_for_status()
