...
async def test_campaign_run_stat_soft_delete(my_setup):
    ...
    async with system_session() as db:
        ...
        # services.metrics.get_spend_discrepancy returns Tuple[Decimal, Decimal, Decimal]
        total_source_spend, total_platform_spend, total_discrepancy = await get_spend_discrepancy(db, company_id, date(2026, 1, 1), date(2026, 1, 2))
        assert total_source_spend == Decimal(0)
