...
class DummyConfig:
    def __init__(self, company_id, connector_name="affise"):
...
async def test_affise_upsert_idempotency(company_b_fixtures):
    ...
    config = DummyConfig(company_id, connector_name="affise")
    connector = AffiseConnector(config, "secret_key")
...
async def test_affise_tenant_isolation(company_b_fixtures):
    ...
    async with system_session() as db_session:
        ...
        await db_session.flush()
        mapping_a = ExternalCampaignMapping(company_id=run_a.company_id, platform="affise", external_id=run_a.note, campaign_run_id=run_a.id)
