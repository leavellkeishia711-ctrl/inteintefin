...
@pytest.mark.asyncio
async def test_create_mapping_success(client_a: AsyncClient, db_session, test_campaign_run_a):
    payload = {
        "platform": "meta",
        "external_id": "test_ext_1",
        "campaign_run_id": str(test_campaign_run_a.id)
    }
    ...
    assert data["campaign_run_id"] == str(test_campaign_run_a.id)
    
@pytest.mark.asyncio
async def test_create_duplicate_returns_409(client_a: AsyncClient, db_session, test_campaign_run_a):
    payload = {
        "platform": "google_ads",
...
@pytest.mark.asyncio
async def test_soft_delete_mapping(client_a: AsyncClient, db_session, test_campaign_run_a):
    payload = {
        "platform": "tiktok_ads",
...
@pytest.mark.asyncio
async def test_get_mappings_filtered(client_a: AsyncClient, db_session, test_campaign_run_a):
    for ext_id in ["f1", "f2"]:
        await client_a.post("/api/v1/external-campaign-mappings/", json={
            "platform": "meta", "external_id": ext_id, "campaign_run_id": str(test_campaign_run_a.id)
        })
    await client_a.post("/api/v1/external-campaign-mappings/", json={
        "platform": "google_ads", "external_id": "f3", "campaign_run_id": str(test_campaign_run_a.id)
    })
...
@pytest.mark.asyncio
async def test_list_isolates_by_company(client_a: AsyncClient, client_b: AsyncClient, test_campaign_run_a, test_campaign_run_b):
    res_a = await client_a.post("/api/v1/external-campaign-mappings/", json={
        "platform": "binom", "external_id": "iso_a", "campaign_run_id": str(test_campaign_run_a.id)
    })
    assert res_a.status_code == 201
    
    res_b = await client_b.post("/api/v1/external-campaign-mappings/", json={
        "platform": "binom", "external_id": "iso_b", "campaign_run_id": str(test_campaign_run_b.id)
    })
