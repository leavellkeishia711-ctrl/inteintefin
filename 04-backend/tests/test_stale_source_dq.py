...
async def test_stale_source_dq(db_session: AsyncSession, company: dict):
    ...
    await db_session.commit() # Commits alerts
    ...
    assert stale_alerts[0].dedup_key == f"connector_stale_dq_{conn.id}"
