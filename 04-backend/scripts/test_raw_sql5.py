...
async def main():
    ...
    async with tenant_session(str(company_a_id)) as db:
        ...
        await db.execute(text(
            "INSERT INTO users (id, company_id, name, email, password_hash, role) VALUES (:id::UUID, :company_id::UUID, :name::VARCHAR, :email, :password_hash::VARCHAR, :role::VARCHAR) RETURNING users.created_at, users.updated_at"
        ), {
            "id": user_a_id, 
        ...
        print("User inserted via raw SQL without email cast")
